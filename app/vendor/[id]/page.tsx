...
const Products = ({ products, productFilters, fetchProduct }) => {
    ...
    const id = params?.id;
    const showLimit = 12, showPagination = 4;
    ...
    useEffect(() => {
        ...
        setSingleStore(data.find((item) => item.id === numericId));
        cratePagination();
    }, [productFilters, limit, pages, products.items.length, id]);

    const cratePagination = () => {
        // set pagination
        let arr = Array.from({ length: Math.ceil(products.items.length / limit) }, (_, idx) => idx + 1);
